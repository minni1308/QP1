...
const Main = () => {
  ...
  // Only show teacher/admin nav if user is logged in
  const nav = token
    ? user?.admin
  ...
  return (
    <>
      ...
      <Routes>
        ...
        {/* Default fallback - redirect to home if not logged in, or appropriate landing page if logged in */}
        <Route path="*" element={({ location }) => {
          ...
          return <Navigate to={user?.admin ? "/admin/home" : "/teacher/landing"} />;
        }} />
      </Routes>
    </>
  );
